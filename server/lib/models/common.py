...
class DownSample(nn.Module):
    ...
    def forward(self, x):
        ...
        if x1.size(2) != x2.size(2):  # In case there's still a mismatch
            x2 = x2[:, :, :x1.size(2), :]  # Crop height dimension to match
...
class PaFPNELAN(nn.Module):
    def __init__(self, 
        ...
        # top dwon
        ## P5 -> P4
        self.cv1 = Conv(c5, 256, k=1, act=act) 
